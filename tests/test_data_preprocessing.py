...
def test_tel_data(tmp_path):
    ...
    assert train_data.shape == (
        2,
        5,
    ), "Les dimensions de train_data ne correspondent pas"
...
def test_preprocess_data():
    ...
    assert isinstance(
        train_submission, pd.Series
    ), "train_submission doit être un Series"
    assert (
        train.shape[1] == test.shape[1]
    ), "Le nombre de colonnes entre train et test doit être le même"
    assert list(train_submission) == [
        1,
        0,
    ], "Les valeurs de train_submission ne sont pas correctes"
